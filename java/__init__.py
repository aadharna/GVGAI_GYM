...
for game in games:
	...
	if(os.path.isdir(gamePath)):
		...
		for lvl in range(lvls):
			for request_image in [True, False]:
				# space_invaders should yield SpaceInvaders-v0 and SpaceInvaders-ram-v0
				name = game.split('_')[0]
				version = int(game.split('_')[-1][1:])
				register(
					id='gvgai-{}-{}-lvl{}-v{}'.format(name, "pixels" if request_image else "nopixels", lvl, version),
					entry_point='gym_gvgai.envs.gvgai_env:GVGAI_Env',
					kwargs={'game': name, 'level': lvl, 'version': version, 'request_image': request_image},    #'obs_type': obs_type
